...
// Original: src/md/render_blocks.zig at commit 6a8f33e7b1 (bun-v1.3.9)
// Modifications: Stripped Bun-specific dependencies for standalone compilation.

pub fn enterBlock(self: *Parser, block_type: BlockType, data: u32, flags: u32) types.CallbackError!void {
    if (self.image_nesting_level > 0) return;
    try self.renderer.enterBlock(block_type, data, flags);
}

pub fn leaveBlock(self: *Parser, block_type: BlockType, data: u32) types.CallbackError!void {
    if (self.image_nesting_level > 0) return;
    try self.renderer.leaveBlock(block_type, data);
}

pub fn processCodeBlock(self: *Parser, block_lines: []const VerbatimLine, data: u32, flags: u32) types.CallbackError!void {
    _ = data;
...
pub fn processHtmlBlock(self: *Parser, block_lines: []const VerbatimLine) types.CallbackError!void {
