...
// Original: src/md/helpers.zig at commit 6a8f33e7b1 (bun-v1.3.9)
...
pub fn decodeUtf8(text: []const u8, off: usize) Utf8DecodeResult {
    ...
    const cp = std.unicode.utf8Decode(text[off..][0..seq_len]) catch 0xFFFD;
...
pub fn encodeUtf8(codepoint: u21, buf: *[4]u8) u3 {
    return std.unicode.utf8Encode(codepoint, buf) catch 0;
}
