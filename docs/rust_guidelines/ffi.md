...
**Checklist:**
- Share only portable types (`#[repr(C)]`, no statics/TypeId, no non-portable pointers).
- Do not move allocations (`String`, `Vec`, `Box<_>`) across DLLs.
- Be cautious with libraries relying on statics (e.g., `tokio`, `log`).
- Treat invisible FFI crossings (methods using foreign types) as unsafe unless portable.
...
Portable here means data that is safe and consistent to process regardless of its origin. By definition, this is a subset of FFI-safe types. A type is portable if it is `#[repr(C)]` (or similarly well-defined), and _all_ of the following:
...
_Interaction_ means any computational relationship, and therefore also relates to how the type is used. Sending a `u128` between DLLs is OK, using it to exchange a transmuted `TypeId` isn't.
...
- Interop rules: `libraries-interop.md`
