...
// Original: src/md/inlines.zig at commit 6a8f33e7b1 (bun-v1.3.9)
...
pub fn processInlineContent(self: *Parser, content: []const u8, base_off: OFF) Parser.Error!void {
    ...
    if (self.recursion_depth >= max_recursion) return error.StackOverflow;
...
pub fn enterSpan(self: *Parser, span_type: SpanType) types.CallbackError!void {
    if (self.image_nesting_level > 0) return;
    try self.renderer.enterSpan(span_type, .{});
}

pub fn leaveSpan(self: *Parser, span_type: SpanType) types.CallbackError!void {
    if (self.image_nesting_level > 0) return;
    try self.renderer.leaveSpan(span_type);
}

pub fn emitText(self: *Parser, text_type: TextType, content: []const u8) types.CallbackError!void {
    try self.renderer.text(text_type, content);
}

/// Emit emphasis opening tags (outermost to innermost).
pub fn emitEmphOpenTags(self: *Parser, sizes: []const u2) types.CallbackError!void {
...
pub fn emitEmphCloseTags(self: *Parser, sizes: []const u2) types.CallbackError!void {
    for (sizes) |size| {
...
pub fn findCodeSpanEnd(self: *const Parser, content: []const u8, start: usize, count: usize) ?usize {
    _ = self;
    var pos = start;
    while (std.mem.indexOfScalarPos(u8, content, pos, '`')) |backtick_pos| {
        pos = backtick_pos + 1;
        while (pos < content.len and content[pos] == '`') pos += 1;
