...
// Original: src/md/types.zig at commit 6a8f33e7b1 (bun-v1.3.9)
...
pub const Renderer = struct {
    ...
    pub const VTable = struct {
        enterBlock: *const fn (ptr: *anyopaque, block_type: BlockType, data: u32, flags: u32) CallbackError!void,
        leaveBlock: *const fn (ptr: *anyopaque, block_type: BlockType, data: u32) CallbackError!void,
        enterSpan: *const fn (ptr: *anyopaque, span_type: SpanType, detail: SpanDetail) CallbackError!void,
        leaveSpan: *const fn (ptr: *anyopaque, span_type: SpanType) CallbackError!void,
        text: *const fn (ptr: *anyopaque, text_type: TextType, content: []const u8) CallbackError!void,
    };

    pub inline fn enterBlock(self: Renderer, block_type: BlockType, data: u32, flags: u32) CallbackError!void {
        return self.vtable.enterBlock(self.ptr, block_type, data, flags);
    }
    pub inline fn leaveBlock(self: Renderer, block_type: BlockType, data: u32) CallbackError!void {
        return self.vtable.leaveBlock(self.ptr, block_type, data);
    }
    pub inline fn enterSpan(self: Renderer, span_type: SpanType, detail: SpanDetail) CallbackError!void {
        return self.vtable.enterSpan(self.ptr, span_type, detail);
    }
    pub inline fn leaveSpan(self: Renderer, span_type: SpanType) CallbackError!void {
        return self.vtable.leaveSpan(self.ptr, span_type);
    }
    pub inline fn text(self: Renderer, text_type: TextType, content: []const u8) CallbackError!void {
...
// Standalone error type replacing CallbackError for Renderer callbacks.
// OutOfMemory is the only error the built-in HtmlRenderer can produce;
// custom renderers may propagate it for any allocation failure.
pub const CallbackError = error{OutOfMemory};
