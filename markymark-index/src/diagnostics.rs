...
//!
//! This logic is shared between the LSP transport (which publishes diagnostics
//! via `textDocument/publishDiagnostics`) and the MCP transport (which exposes
//! a `get_diagnostics` tool).

use std::collections::HashMap;

use markymark_core::engine::{CoreDiagnostic, DiagnosticSeverity};
...
/// Compute diagnostics for a single document given its index and the realm it belongs to.
///
/// Checks for:
/// - Broken wiki links (target page or heading doesn't exist)
/// - Broken markdown link anchors (heading slug doesn't exist in target)
        ...
        if let Some(anchor) = &ml.anchor {
            ...
            if resolved.is_none() {
                diagnostics.push(CoreDiagnostic {
                    range: ml.range,
                    severity: DiagnosticSeverity::Error,
                    message: format!("Broken link: heading '{anchor}' not found"),
                });
            }
        }
    }
