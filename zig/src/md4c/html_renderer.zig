...
// Original: src/md/html_renderer.zig at commit 6a8f33e7b1 (bun-v1.3.9)
...
pub const HtmlRenderer = struct {
    ...
    fn enterBlockImpl(ptr: *anyopaque, block_type: BlockType, data: u32, flags: u32) types.CallbackError!void {
        const self: *HtmlRenderer = @ptrCast(@alignCast(ptr));
        self.enterBlock(block_type, data, flags);
    }

    fn leaveBlockImpl(ptr: *anyopaque, block_type: BlockType, data: u32) types.CallbackError!void {
        const self: *HtmlRenderer = @ptrCast(@alignCast(ptr));
        self.leaveBlock(block_type, data);
    }

    fn enterSpanImpl(ptr: *anyopaque, span_type: SpanType, detail: SpanDetail) types.CallbackError!void {
        const self: *HtmlRenderer = @ptrCast(@alignCast(ptr));
        self.enterSpan(span_type, detail);
    }

    fn leaveSpanImpl(ptr: *anyopaque, span_type: SpanType) types.CallbackError!void {
        const self: *HtmlRenderer = @ptrCast(@alignCast(ptr));
        self.leaveSpan(span_type);
    }

    fn textImpl(ptr: *anyopaque, text_type: TextType, content: []const u8) types.CallbackError!void {
        const self: *HtmlRenderer = @ptrCast(@alignCast(ptr));
    ...
    fn matchTagNameCI(content: []const u8, pos: usize, tag: []const u8) bool {
        ...
        if (!std.ascii.eqlIgnoreCase(content[pos..][0..tag.len], tag)) return false;
